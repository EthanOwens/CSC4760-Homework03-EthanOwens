...
int main(int argc, char *argv[]) {
    ...
    Kokkos::View<int*> x_linear("linear x", M);

    if(rank == 0){
        Kokkos::parallel_for("fill x linear", M, KOKKOS_LAMBDA(int i){
            x_linear(i) = i;
        });
    }
    Kokkos::fence();

    Kokkos::View<int*> local_x_linear("linear x Local", ele_per_proc);
    Kokkos::parallel_for("linearly distribute x", ele_per_proc, KOKKOS_LAMBDA(int i) {
        local_x_linear(i) = x_linear(i * Q + my_col);
    });
    Kokkos::fence();
    ...
    MPI_Bcast(local_x_linear.data(), ele_per_proc, MPI_INT, col, MPI_COMM_WORLD);
